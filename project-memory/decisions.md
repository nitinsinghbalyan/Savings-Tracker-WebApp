...
| 2026-09-07 | A fifth tab **Worth**, superseding the 2026-06-14 "Bottom navigation (4 tabs)" and session 81's four-pill segmented control | Net worth is a top-level view checked often; burying it under Settings would hide a screen the user opens deliberately. Segmented pill text drops `text-xs` → `text-[11px]` so five fit at 375px | A collapsible block inside Summary (would lengthen the default tab); a nested `/settings/net-worth` route |
| 2026-09-07 | One `holdings` table with a `holding_group` CHECK, not separate assets/liabilities tables | Assets and liabilities differ only in sign and grouping; one table keeps aggregation a single pass, the add/edit form one component, and archiving/sorting uniform | Two tables mirroring the spreadsheet's sheets; a generic key-value store |
| 2026-09-07 | Liquid assets and short-term liabilities are read **live** from `accounts`, never copied into `holdings` | The ledger already maintains those balances via `get_account_balances()`; duplicating them would drift the moment a transaction lands. `linked_account_id` on a holding makes it skip the manual pass, so a linked item is counted once | Fully manual entry mirroring the spreadsheet (goes stale); auto-syncing account balances into holding rows (drift + write amplification) |
| 2026-09-07 | Net worth history comes from `net_worth_snapshots` (capped 24 rows), written client-side once per period on Worth-tab activation | This is the direct fix for F-87, removed in session 46 for reconstructing history by walking the entire ledger backwards. Snapshot reads are O(months) and survive later edits to holdings | Replaying the ledger (the removed F-87 approach); a scheduled server job (no backend beyond Supabase); storing history on each holding |
| 2026-09-07 | `contribution_plans` is a new table rather than reusing `categories.monthly_budget` | Budgets are spending **ceilings** consumed by the Summary expense heatmap (`monthlySummary.js:69-74`); contribution plans are savings **floors**. Overloading the column would corrupt budget maths | Reuse `monthly_budget` with an `is_savings` filter; derive the plan from `recurring_transactions` |
| 2026-09-07 | Net worth data is **not** added to the `AppDataContext` bootstrap `Promise.all` | That bootstrap already fires five requests before first paint and sessions 75-76 were spent recovering that time (F-135/F-136). `useNetWorth` fetches on first Worth-tab activation; the Summary card calls it with `enabled: false` and renders from cache only, gated on `loaded` so it never shows an accounts-only figure as if it were net worth | Add a sixth bootstrap fetch; fetch eagerly on idle like the chunk prefetch (still competes with first paint) |
| 2026-09-07 | The target lives on `user_profiles` as two nullable columns, not its own table | There is exactly one target per user, and `getProfile()` already does `select('*')` so the columns flow through with no query change | A `net_worth_targets` table; multiple concurrent targets |
| 2026-09-20 | Bottom bar returns for three high-traffic destinations plus Add, **partially reversing** the 2026-09-05 "mobile nav moves to a top segmented control" decision | User asked for navigation at the bottom. The reversal is partial and deliberate: `SegmentedTabs` keeps full navigation to all five screens at the top, and the bottom bar is a quick-access strip (Month / Worth / + / Ledger), so the bottom edge is re-occupied by choice rather than necessity | Replace the top control outright (loses one-tap access to Goals and More); a four-destination bar (would centre the +, but the user chose three); actions-only bar with no destinations |
| 2026-09-20 | The centre **+** replaces the Ledger FAB rather than sitting alongside it | The FAB rendered at `z-[60]`, *above* the `z-50` modal overlay, and was not gated on `formOpen`, so it floated over the open transaction sheet. It also caused the 2026-06-14 "FAB not tappable under the install banner" incident. One Add affordance, correctly stacked, removes both | Keep the FAB and add the + as well (two Add buttons); keep the FAB and omit + from the bar |
| 2026-09-20 | The + opens the form by navigating to `/transactions?new=1`, and `TransactionsPage` **derives** openness from the param rather than syncing it in an effect | The bar lives in `AppShell`, outside the page that owns the form, so it needs a cross-tab channel. `?month=` already drives state in this file, so the URL was the established seam. Deriving (`formOpen \|\| addFromUrl`) instead of calling `openAddForm()` in an effect avoids a cascading render and the `react-hooks/set-state-in-effect` error | An `openAddTransaction` callback on `ShellChromeContext` (more plumbing, more coupling); an effect syncing the param into state (lint error, extra render) |
| 2026-09-20 | Adopt the "Goal Tracker Mobile" redesign **partially** — visual language and screen content, but **not** its navigation | Every artboard drops the bottom bar, moves ＋ to the header, and uses 4 top tabs (Overview · Insights · Goals · Ledger) with Worth as a sub-screen. That would revert F-150, shipped the day before at the user's explicit request. Asked rather than assumed; the user chose to keep the bar | Follow the artboards exactly (reverts the bottom bar and reopens the FAB question); keep the bar but adopt the design's 4-tab set (Worth would stop being a route) |
| 2026-09-20 | Insights (1b) and Suggestions (1a) deferred | Insights needs income-vs-spend over 1M/3M/6M/1Y and "Sep vs Aug by category" — multi-month history, i.e. the unbounded-ledger shape that got F-87 removed in session 46. It wants monthly aggregate snapshots, not a wider transaction fetch. Suggestions binds `s.tag/title/body/action` with no data source or rules — a recommendation engine, not a layout | Build both now (large, and Suggestions has undefined behaviour); fake Suggestions with static copy (ships a screen that cannot update) |
| 2026-09-20 | Overview deltas use a **second month-scoped** query, gated on the visible month having painted | The badges need last month's totals and there is no aggregate to read them from. One extra bounded, indexed month query is acceptable where an all-time fetch is not. Gating on `!initialLoading` keeps it off the first-paint path that F-135/F-136 were spent recovering | Widen the existing fetch to two months (couples the headline to data it does not need); store monthly aggregates (right answer for Insights later, overkill for three badges); drop the badges |
| 2026-09-20 | Goals / goal-detail / ledger aggregates are all computed **per currency** from data already in the cache | Consistent with the 2026-06-14 no-FX decision: the artboard's "3 funded" counts a USD goal that must not enter an INR percentage. And none of the three screens may widen a query — the ledger chart and the funding chart both read what is already loaded | Blend currencies for a single headline number (misleading); fetch wider history for richer charts (the F-87 failure) |
| 2026-09-20 | Log sheet (1f) **not** rebuilt as a single-step sheet | The artboard contradicts F-141 (two-step keypad → details, session 80). The ship skill requires raising a logged-decision conflict rather than reversing it quietly. The existing form already covers every capability the artboard shows, including "To goal" via the F-139 goal picker | Rebuild as one sheet (reverses F-141); take only the additive parts — Today/Yesterday shortcuts, account balance hint, recurring affordance |
| 2026-09-20 | Settings Appearance (dark mode) treated as its own feature, not part of this redesign | Adding Light/Dark/Auto means a full dark palette in `tailwind.config.js` and auditing every screen's paper/ink tokens. Shipping a toggle without that is worse than no toggle. Likewise a "Daily suggestions" switch would gate a feature that is deferred | Build dark mode inside the redesign pass (large, and would go unverified); ship a non-functional toggle (dead UI) |
| 2026-09-20 | Dark mode by **inverting the neutral ramp via CSS variables**, not by migrating ~619 utilities | The app is mid-migration from legacy slate/brand to paper-and-ink and stalled ~29% in. Defining the neutrals as custom properties and inverting them under `.dark` makes the ~394 un-migrated `slate` utilities adapt with no call-site edits; the alternative was a week of rewrites across 33 files | Rewrite every utility to design tokens (correct end state, very large); ship a toggle with only the shell themed (most screens visibly broken); skip dark mode |
| 2026-09-20 | Colour variables store **RGB channel triples**, not hex, and the config wraps them in `rgb(var(--x) / <alpha-value>)` | The first attempt stored hex and broke the build: Tailwind cannot compute an opacity modifier from `var(--brand-500)`, so `focus:ring-brand-500/20` failed to compile. 18 alpha modifiers exist across `brand`/`surface`/`accent`/`slate`/`negative`; the channel form preserves all of them | Store hex and drop the opacity modifiers (18 call sites, loses translucency); define separate `--x-alpha` variables per usage (unmaintainable) |
| 2026-09-20 | Tailwind's `white` deliberately **left un-themed**; the 27 `bg-white` surfaces rewritten to a new `surface` token instead | Theming `white` would have fixed all 27 surfaces and simultaneously broken all 35 `text-white` labels, which sit on accent buttons and must stay white. A bounded 27-occurrence rename was the cheaper and safer half of the trade | Theme `white` and add a `text-on-accent` escape hatch (35 call sites, and every future `text-white` is a latent bug) |
| 2026-09-20 | The ink alpha tiers derive from **one `--ink` triple** rather than five independent colours | Inverting a single channel triple carries `ink-muted/soft/faint/rule/hairline` with it automatically, so light and dark cannot drift apart. Costs a little per-tier control in dark | Five separate variables per theme (ten values to keep in sync) |
