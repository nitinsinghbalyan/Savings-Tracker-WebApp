...
- **`add_transaction_category_snapshot.sql` not applied** — create/update transaction may fail (unknown columns) or category delete still drops labels until migration + session 63 app code run.
...
- **Local vs remote `master` divergence** — GitHub `master` may be at `3b3344f` while local is `550a09e`; push may require merge/rebase or force (avoid force to main without explicit request).
