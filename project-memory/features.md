...
| F-156 | Log sheet redesign (1f) | deferred | Artboard is a single-step sheet; the app's form is a deliberate two-step wizard (**F-141**, session 80). Raised with the user rather than reversed — awaiting a decision (session 86) |
| F-157 | Settings Appearance / dark mode (1g) | deferred | Artboard adds Light/Dark/Auto. The app has no dark mode and `tailwind.config.js` no dark palette — a feature in its own right, not a restyle. Settings was already converted in F-145 (session 86) |
